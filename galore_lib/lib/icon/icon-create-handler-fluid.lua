vgal.icon.create_handlers["fluid"] = function(blueprint)
    ...
    table.insert(composites, vgal.icon.shift(vgal.icon.get(blueprint.inputs[1]), 0.7, { 0, -6.5 }))

    if blueprint.inputs[2] then
        table.insert(composites, vgal.icon.shift(vgal.icon.get(blueprint.inputs[2]), 0.5, { 0, -5 }))
    end
