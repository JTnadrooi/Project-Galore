function vgal.recipe.all.link_ingredient(ingredient, main_product_name, multiplier, search_table) -- vgal only
...
function vgal.recipe.all.link_result(result, main_product_name, multiplier, search_table) -- vgal only
    search_table = search_table or data.raw["recipe"]
    multiplier = multiplier or 1
    for _, recipe in pairs(search_table) do
        if (main_product_name == nil) or recipe.main_product == main_product_name then
...
function vgal.recipe.all.multiply_results(main_product_name, multiplier, search_table) -- vgal only
    search_table = search_table or data.raw["recipe"]
    multiplier = multiplier or 1
    for _, recipe in pairs(search_table) do
        if (main_product_name == nil) or recipe.main_product == main_product_name then
...
function vgal.recipe.all.multiply_ingredients(main_product_name, multiplier, ingredient_name, search_table) -- vgal only
    search_table = search_table or data.raw["recipe"]
    multiplier = multiplier or 1
    for _, recipe in pairs(search_table) do
        if (main_product_name == nil) or recipe.main_product == main_product_name then
