...
function vgal.recipe.make_recipeable_void(recipeable_name, void_category, void_amount)
    ...
    local input_type
    local base_data
    if data.raw.fluid[recipeable_name] then
        input_type = "fluid"
        base_data = data.raw.fluid[recipeable_name]
    elseif data.raw.item[recipeable_name] or data.raw["capsule"][recipeable_name] or data.raw["tool"][recipeable_name] then
        input_type = "item"
        base_data = vgal.get_recipeable(recipeable_name)
    else
        error(recipeable_name)
    end

    local cat = void_category
    local configs = {
        water = {
            default_amount = 400,
            process_time = 5,
            output_item = "angels-water-void",
        ...
        chemical = {
            default_amount = 100,
            process_time = 1,
            output_item = "angels-chemical-void",
            output_probability = 0,
            tint = true,
        },
        bio = {
    ...
    local output_amount
    if cat == "bio" then
        output_amount = amount < 1 and 1 / amount or 1
    else
        output_amount = amount < 1 and amount or 1
    end

    local subgroup = base_data.subgroup or "angels-void"
    local process_time = cfg.process_time
    local output_item = cfg.output_item
    local output_prob = cfg.output_probability
    ...
    local recipe = {
        ...
        subgroup = "angels-" .. cat .. "-void",
        order = "",
        icons = {},
    }

    local group_order = data.raw["item-group"][data.raw["item-subgroup"][subgroup].group].order or "z"
    local subgroup_order = data.raw["item-subgroup"][subgroup].order
    local object_order = base_data.order or "z"
    recipe.order = string.format("%s-%s-%s", group_order, subgroup_order, object_order)
    if #recipe.order > 200 then recipe.order = recipe.order:sub(1, 200) end

    local icons = util.table.deepcopy(
        angelsmods.functions.get_object_icons(output_item) or { { icon = "__angelsrefininggraphics__/graphics/icons/void.png", icon_size = 32 } }
    )
    local fluid_icon = util.table.deepcopy(angelsmods.functions.get_object_icons(recipeable_name) or {})
