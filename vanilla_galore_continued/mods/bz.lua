if mods["bzcarbon"] or mods["bztin"] then
    vgal.recipe.all.multiply_results("electronic-circuit", 2, vgal.recipes)
...
if mods["bzcarbon"] then
    ...
    if settings.startup["bzcarbon-reuse"].value == "yes" then
        vgal.recipe.all.link_result({ type = "item", name = "diamond", amount = 1, probability = 0.5 },
            "low-density-structure", 1, vgal.recipes)
    end
    ...
    vgal.recipe.all.link_ingredient({ type = "item", name = "graphite", amount = 1 }, "electronic-circuit", 0.5,
        vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "graphite", amount = 1 }, "battery", 1, vgal.recipes)
...
if mods["bzsilicon"] then
    ...
    if mods["bzcarbon"] then
        vgal.recipe.all.replace_ingredient("stone-brick", { type = "item", name = "crucible", amount = 1 },
            "electric-furnace", 0.1, vgal.recipes)
    end
end
...
if mods["bzlead"] then
    ...
    if settings.startup["bzlead-more-entities"].value == "yes" then
        vgal.recipe.all.link_ingredient({ type = "item", name = "lead-expansion-bolt", amount = 4 },
            "assembling-machine-1", 1, vgal.recipes)
    ...
    vgal.recipe.all.replace_ingredient("iron-plate", { type = "item", name = "lead-plate", amount = 1 },
        "uranium-fuel-cell", 1, vgal.recipes)

    vgal.recipe.deep_hide("vgal-steel-plate-uranium-fuel-cell")
end

if mods["bztin"] then
    vgal.recipe.all.link_ingredient({ type = "item", name = "tin-plate", amount = 5 }, "lab", 1, vgal.recipes)
    ...
    vgal.recipe.all.link_ingredient({ type = "item", name = "solder", amount = 1 }, "electronic-circuit", 1,
        vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "solder", amount = 1 }, "advanced-circuit", 1, vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "solder", amount = 2 }, "substation", 1, vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "solder", amount = 2 }, "accumulator", 1, vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "solder", amount = 20 }, "roboport", 1, vgal.recipes)
...
if mods["bztitanium"] then
    vgal.recipe.all.replace_ingredient("steel-plate", { type = "item", name = "titanium-plate", amount = 1 },
        "low-density-structure", 1, vgal.recipes)
    vgal.recipe.all.replace_ingredient("steel-plate", { type = "item", name = "titanium-plate", amount = 1 },
        "flying-robot-frame", 1, vgal.recipes)

    if mods["space-age"] then
        vgal.recipe.all.link_ingredient({ type = "item", name = "titanium-plate", amount = 2 },
            "space-platform-foundation", 1, vgal.recipes)
...
if mods["bzzirconium"] then
    vgal.recipe.all.link_ingredient({ type = "item", name = "zirconia", amount = 10 }, "low-density-structure", 1,
        vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "zirconia", amount = 6 }, "steel-furnace", 1, vgal.recipes)
    vgal.recipe.all.link_ingredient({ type = "item", name = "zirconia", amount = 1 }, "explosives", 0.1, vgal.recipes)

    local ingredient = mods["bztin"] and "zircaloy-4" or "zirconium-plate"

    vgal.recipe.all.link_ingredient({ type = "item", name = ingredient, amount = 1 }, "nuclear-fuel", 1, vgal.recipes)
